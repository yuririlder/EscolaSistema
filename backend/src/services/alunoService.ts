...
class AlunoService {
  async criar(data: any) {
    ...
    if (data.cpf) {
      const cpfExists = await queryOne('SELECT id FROM alunos WHERE cpf = $1', [data.cpf]);
      if (cpfExists) {
    ...
    const responsavel = await queryOne('SELECT id FROM responsaveis WHERE id = $1', [data.responsavel_id]);
    if (!responsavel) {
  ...
  async vincularTurma(alunoId: string, turmaId: string) {
    ...
    const turma = await queryOne('SELECT id FROM turmas WHERE id = $1', [turmaId]);
    if (!turma) {
      throw new Error('Turma não encontrada');
    }

    await query(
      "UPDATE alunos SET turma_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2",
      [turmaId, alunoId]
    );
    
    logger.info(`Aluno ${alunoId} vinculado à turma ${turmaId}`);
    return this.buscarPorId(alunoId);
  }

  async desvincularTurma(alunoId: string) {
    await query(
      "UPDATE alunos SET turma_id = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = $1",
      [alunoId]
    );
    
    logger.info(`Aluno ${alunoId} desvinculado da turma`);
    return this.buscarPorId(alunoId);
  }
}
