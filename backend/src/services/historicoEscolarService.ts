...
class HistoricoEscolarService {
  ...
  async vincularAlunoTurma(data: {
    ...
    const aluno = await queryOne('SELECT id FROM alunos WHERE id = $1', [data.aluno_id]);
    if (!aluno) {
      throw new Error('Aluno não encontrado');
    }

    // Verificar se turma existe
    const turma = await queryOne('SELECT id, ano FROM turmas WHERE id = $1', [data.turma_id]);
    if (!turma) {
      throw new Error('Turma não encontrada');
    }

    // Verificar se já existe vínculo para este ano letivo
    const vinculoExistente = await queryOne(
      'SELECT id, turma_id FROM historico_escolar WHERE aluno_id = $1 AND ano_letivo = $2',
    ...
    if (vinculoExistente) {
      ...
      await query(
        `UPDATE historico_escolar 
         ...
         WHERE id = $3`,
        [data.turma_id, data.observacoes, vinculoExistente.id]
      );
      
      // Atualizar turma_id na tabela alunos se for o ano atual
      const anoAtual = new Date().getFullYear();
      if (data.ano_letivo === anoAtual) {
        await query('UPDATE alunos SET turma_id = $1 WHERE id = $2', [data.turma_id, data.aluno_id]);
      }
      
      logger.info(`Vínculo aluno-turma atualizado: ${data.aluno_id} -> ${data.turma_id} (${data.ano_letivo})`);
    ...
    const id = uuidv4();
    await query(
      `INSERT INTO historico_escolar (id, aluno_id, turma_id, ano_letivo, status, observacoes)
       VALUES ($1, $2, $3, $4, 'CURSANDO', $5)`,
      [id, data.aluno_id, data.turma_id, data.ano_letivo, data.observacoes]
    );
    ...
    logger.info(`Aluno ${data.aluno_id} vinculado à turma ${data.turma_id} no ano letivo ${data.ano_letivo}`);
   ...
   */
  async desvincularAlunoTurma(alunoId: string, anoLetivo: number) {
    const historico = await queryOne(
      'SELECT id FROM historico_escolar WHERE aluno_id = $1 AND ano_letivo = $2',
      [alunoId, anoLetivo]
    );
    ...
    if (anoLetivo === anoAtual) {
      await query('UPDATE alunos SET turma_id = NULL WHERE id = $1', [alunoId]);
    }

    logger.info(`Aluno ${alunoId} desvinculado do ano letivo ${anoLetivo}`);
  }

  /**
   * Buscar histórico completo de um aluno
   */
  async buscarHistoricoAluno(alunoId: string) {
    ...
    const historicoCompleto = await Promise.all(
      historico.map(async (h: any) => {
        ...
        const professores = await queryMany(
          `SELECT tp.disciplina, f.nome as professor_nome, p.id as professor_id
   ...
   */
  async buscarVinculoAtual(alunoId: string) {
    const anoAtual = new Date().getFullYear();
    return await queryOne(
      `SELECT he.*, 
              t.nome as turma_nome, t.serie as turma_serie, t.turno as turma_turno
       FROM historico_escolar he
       INNER JOIN turmas t ON he.turma_id = t.id
       WHERE he.aluno_id = $1 AND he.ano_letivo = $2 AND he.status = 'CURSANDO'`,
      [alunoId, anoAtual]
    );
  }
   ...
   */
  async listarAlunosPorTurmaAno(turmaId: string, anoLetivo: number) {
    return await queryMany(
      `SELECT he.*, a.nome as aluno_nome, a.data_nascimento, a.cpf,
   ...
   */
  async atualizarStatus(id: string, status: string, observacoes?: string) {
    ...
    logger.info(`Status do histórico ${id} atualizado para ${status}`);
    return this.buscarPorId(id);
  }
   ...
   */
  async buscarTodos(anoLetivo?: number) {
    const sql = anoLetivo
      ? `SELECT he.*, 
                t.nome as turma_nome, t.serie as turma_serie, t.turno as turma_turno,
                a.nome as aluno_nome
    ...
    return anoLetivo 
      ? await queryMany(sql, [anoLetivo])
      : await queryMany(sql);
  }
}

export const historicoEscolarService = new HistoricoEscolarService();
