...
class FinanceiroService {
    ...
    async criarPlano(data) {
        ...
        await (0, connection_1.query)(`INSERT INTO planos_mensalidade (id, nome, descricao, valor, ativo)
       VALUES ($1, $2, $3, $4, $5)`, [id, data.nome, data.descricao, data.valor, data.ativo !== false]);
        logger_1.logger.info(`Plano criado: ${data.nome}`);
        return this.buscarPlanoPorId(id);
    }
    async listarPlanos() {
        return await (0, connection_1.queryMany)('SELECT * FROM planos_mensalidade ORDER BY nome ASC');
    }
    async listarPlanosAtivos() {
        return await (0, connection_1.queryMany)('SELECT * FROM planos_mensalidade WHERE ativo = true ORDER BY nome ASC');
    }
    async buscarPlanoPorId(id) {
        return await (0, connection_1.queryOne)('SELECT * FROM planos_mensalidade WHERE id = $1', [id]);
    }
    async atualizarPlano(id, data) {
        ...
        values.push(id);
        await (0, connection_1.query)(`UPDATE planos_mensalidade SET ${fields.join(', ')} WHERE id = $${paramIndex}`, values);
        return this.buscarPlanoPorId(id);
    }
    ...
    async realizarMatricula(data) {
        ...
        const aluno = await (0, connection_1.queryOne)('SELECT id, nome FROM alunos WHERE id = $1', [data.aluno_id]);
        if (!aluno) {
        ...
        try {
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'ATIVA', $9, $10)`, [matriculaId, data.aluno_id, data.plano_id, data.ano_letivo, data.valor_matricula, valorMensalidade, data.dia_vencimento || 10, dataMatricula, data.desconto || 0, data.observacoes]);
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'PENDENTE')`, [(0, uuid_1.v4)(), data.aluno_id, matriculaId, mes, anoAtual, valorMensalidade, data.desconto || 0, dataVencimento]);
            }
    ...
    async listarMatriculas() {
        return await (0, connection_1.queryMany)(`SELECT m.*, a.nome as aluno_nome, p.nome as plano_nome
       FROM matriculas m
    ...
    async buscarMatriculaPorId(id) {
        return await (0, connection_1.queryOne)(`SELECT m.*, a.nome as aluno_nome, p.nome as plano_nome
       FROM matriculas m
       INNER JOIN alunos a ON m.aluno_id = a.id
       INNER JOIN planos_mensalidade p ON m.plano_id = p.id
       WHERE m.id = $1`, [id]);
    }
    async atualizarMatricula(id, data) {
        ...
        fields.push(`updated_at = CURRENT_TIMESTAMP`);
        values.push(id);
        await (0, connection_1.query)(`UPDATE matriculas SET ${fields.join(', ')} WHERE id = $${paramIndex}`, values);
        logger_1.logger.info(`Matrícula atualizada: ${id}`);
        return this.buscarMatriculaPorId(id);
    }
    async cancelarMatricula(id) {
        ...
        await (0, connection_1.query)("UPDATE matriculas SET status = 'CANCELADA', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [id]);
        await (0, connection_1.query)("UPDATE alunos SET matricula_ativa = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1", [matricula.aluno_id]);
        // Cancelar mensalidades pendentes
        await (0, connection_1.query)("UPDATE mensalidades SET status = 'CANCELADO', updated_at = CURRENT_TIMESTAMP WHERE matricula_id = $1 AND status = 'PENDENTE'", [id]);
        logger_1.logger.info(`Matrícula cancelada: ${id}`);
        return this.buscarMatriculaPorId(id);
    }
    // ==================== MENSALIDADES ====================
    ...
    async registrarPagamentoMensalidade(id, data) {
        ...
        await (0, connection_1.query)("UPDATE mensalidades SET status = 'PAGO', valor_pago = $1, data_pagamento = $2, forma_pagamento = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4", [data.valor_pago || mensalidade.valor, dataPagamento, data.forma_pagamento, id]);
    ...
    async criarDespesa(data) {
        const id = (0, uuid_1.v4)();
        await (0, connection_1.query)(`INSERT INTO despesas (id, descricao, categoria, valor, data_vencimento, fornecedor, observacoes, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'PENDENTE')`, [id, data.descricao, data.categoria, data.valor, data.data_vencimento, data.fornecedor, data.observacoes]);
        logger_1.logger.info(`Despesa criada: ${data.descricao}`);
    ...
    async buscarDespesaPorId(id) {
        return await (0, connection_1.queryOne)('SELECT * FROM despesas WHERE id = $1', [id]);
    }
    async atualizarDespesa(id, data) {
        ...
        fields.push(`updated_at = CURRENT_TIMESTAMP`);
        values.push(id);
        await (0, connection_1.query)(`UPDATE despesas SET ${fields.join(', ')} WHERE id = $${paramIndex}`, values);
        return this.buscarDespesaPorId(id);
    }
    async pagarDespesa(id, data) {
        const dataPagamento = new Date().toISOString().split('T')[0];
        const formaPagamento = data?.forma_pagamento || 'PIX';
        await (0, connection_1.query)("UPDATE despesas SET status = 'PAGO', data_pagamento = $1, forma_pagamento = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $2", [dataPagamento, id, formaPagamento]);
        return this.buscarDespesaPorId(id);
    }
    ...
    async criarPagamentoFuncionario(data) {
        ...
        await (0, connection_1.query)(`INSERT INTO pagamentos_funcionarios (id, funcionario_id, mes_referencia, ano_referencia, salario_base, bonus, descontos, valor_liquido, status, observacoes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'PENDENTE', $9)`, [id, data.funcionario_id, data.mes_referencia, data.ano_referencia, data.salario_base, data.bonus || 0, data.descontos || 0, valorLiquido, data.observacoes]);
        logger_1.logger.info(`Pagamento criado para funcionário ${data.funcionario_id}`);
        return this.buscarPagamentoPorId(id);
    }
    async gerarPagamentosMes(mes, ano) {
        const funcionarios = await (0, connection_1.queryMany)('SELECT id, salario FROM funcionarios WHERE ativo = true');
        const pagamentos = [];
        for (const func of funcionarios) {
            // Verificar se já existe pagamento
            const existe = await (0, connection_1.queryOne)('SELECT id FROM pagamentos_funcionarios WHERE funcionario_id = $1 AND mes_referencia = $2 AND ano_referencia = $3', [func.id, mes, ano]);
            if (!existe) {
    ...
    async listarPagamentosFuncionarios(filtros) {
        let sql = `
    ...
    async buscarPagamentoPorId(id) {
        return await (0, connection_1.queryOne)(`SELECT pf.*, f.nome as funcionario_nome, f.cargo
       FROM pagamentos_funcionarios pf
       INNER JOIN funcionarios f ON pf.funcionario_id = f.id
       WHERE pf.id = $1`, [id]);
    }
    async registrarPagamentoFuncionario(id, data) {
        const dataPagamento = new Date().toISOString().split('T')[0];
        const formaPagamento = data?.forma_pagamento || 'TRANSFERENCIA';
        await (0, connection_1.query)("UPDATE pagamentos_funcionarios SET status = 'PAGO', data_pagamento = $1, forma_pagamento = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $2", [dataPagamento, id, formaPagamento]);
        return this.buscarPagamentoPorId(id);
    }
    // ==================== DASHBOARD ====================
    async obterDashboard() {
        ...
        const totalAlunos = await (0, connection_1.queryOne)('SELECT COUNT(*) as count FROM alunos', []);
        const totalProfessores = await (0, connection_1.queryOne)('SELECT COUNT(*) as count FROM professores p INNER JOIN funcionarios f ON p.funcionario_id = f.id WHERE f.ativo = true', []);
        const totalTurmas = await (0, connection_1.queryOne)('SELECT COUNT(*) as count FROM turmas WHERE ativa = true', []);
        const receitaMes = await (0, connection_1.queryOne)(`SELECT COALESCE(SUM(valor_pago), 0) as total FROM mensalidades 
       ...
       GROUP BY 
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const receita = await (0, connection_1.queryOne)(`SELECT COALESCE(SUM(valor_pago), 0) as total FROM mensalidades 
         WHERE status = 'PAGO' AND EXTRACT(MONTH FROM data_pagamento::date) = $1 AND EXTRACT(YEAR FROM data_pagamento::date) = $2`, [mes, ano]);
            const despesa = await (0, connection_1.queryOne)(`SELECT COALESCE(SUM(valor), 0) as total FROM despesas 
         WHERE status = 'PAGO' AND EXTRACT(MONTH FROM data_pagamento::date) = $1 AND EXTRACT(YEAR FROM data_pagamento::date) = $2`, [mes, ano]);
        ...
        return {
            ...
            alunosPorTurma: alunosPorTurma.map((r) => ({ turma: r.turma, quantidade: parseInt(r.quantidade) })),
            mensalidadesPorStatus: mensalidadesPorStatus.map((r) => ({ status: r.status, quantidade: parseInt(r.quantidade) })),
            receitaVsDespesa
        };
    }
    async obterHistoricoAnual(ano) {
        const meses = [];
        for (let mes = 1; mes <= 12; mes++) {
            const receita = await (0, connection_1.queryOne)(`SELECT COALESCE(SUM(valor_pago), 0) as total FROM mensalidades 
         WHERE status = 'PAGO' AND EXTRACT(MONTH FROM data_pagamento) = $1 AND EXTRACT(YEAR FROM data_pagamento) = $2`, [mes, ano]);
            const despesa = await (0, connection_1.queryOne)(`SELECT COALESCE(SUM(valor), 0) as total FROM despesas 
         WHERE status = 'PAGO' AND EXTRACT(MONTH FROM data_pagamento) = $1 AND EXTRACT(YEAR FROM data_pagamento) = $2`, [mes, ano]);
            meses.push({
                mes,
...
exports.financeiroService = new FinanceiroService();
